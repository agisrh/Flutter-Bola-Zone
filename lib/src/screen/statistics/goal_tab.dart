...
class GoalTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child:
          BlocBuilder<TopScorerBloc, TopScorerState>(builder: (context, state) {
        if (state is TopScorerLoading) {
          return const Center(
            child: CircularProgressIndicator(),
          );
        }
        if (state is TopScorerSuccess) {
          return ListView.builder(
            physics: const BouncingScrollPhysics(),
            padding: EdgeInsets.symmetric(horizontal: 10.w),
            itemCount: state.listTopScorer.length,
            itemBuilder: (BuildContext context, int index) {
              PlayerStatisticModel data = state.listTopScorer[index];
              return Column(
                children: [
                  Row(
