...
class RedCardTab extends StatelessWidget {
  const RedCardTab({super.key});

  @override
  Widget build(BuildContext context) {
    return Container(
      margin: EdgeInsets.only(top: 10.h),
      child: BlocBuilder<RedCardBloc, RedCardState>(builder: (context, state) {
        if (state is RedCardLoading) {
          return const Center(
            child: CircularProgressIndicator(),
          );
        }
        if (state is RedCardSuccess) {
          return ListView.builder(
            physics: const BouncingScrollPhysics(),
            padding: EdgeInsets.symmetric(horizontal: 10.w),
            itemCount: state.listRedCards.length,
            itemBuilder: (BuildContext context, int index) {
              PlayerStatisticModel data = state.listRedCards[index];
              return Column(
                children: [
                  Row(
