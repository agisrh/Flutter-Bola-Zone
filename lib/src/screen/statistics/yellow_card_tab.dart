...
class YellowCardTab extends StatelessWidget {
  const YellowCardTab({super.key});

  @override
  Widget build(BuildContext context) {
    return Container(
      margin: EdgeInsets.only(top: 10.h),
      child: BlocBuilder<YellowCardBloc, YellowCardState>(
          builder: (context, state) {
        if (state is YellowCardLoading) {
          return const Center(
            child: CircularProgressIndicator(),
          );
        }
        if (state is YellowCardSuccess) {
          return ListView.builder(
            physics: const BouncingScrollPhysics(),
            padding: EdgeInsets.symmetric(horizontal: 10.w),
            itemCount: state.listYellowCards.length,
            itemBuilder: (BuildContext context, int index) {
              PlayerStatisticModel data = state.listYellowCards[index];
              return Column(
                children: [
                  Row(
